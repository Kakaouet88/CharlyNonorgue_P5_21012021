...
const fetchProduct = async () => {
  product = await fetch(apiUrl + "/api/teddies/" + productID).then((res) =>
    res.json()
  );
  console.log(product);
  return product;
...
const displayProduct = async () => {
    ...
    <div class="row">
      ...
      product.name
          ...
          <h1 id="productPageName" class="h3">${product.name}</h1>
            <p id="productPagePrice" class="font-weight-bold">${numberWithCommas(
              product.price
            )} €</p>
            <p id="prodctPageDescription" class="pt-1">${
              product.description
            }</p>
  ...
  for (let color of product.colors) {
    ...
    document.getElementById("colorPickerForm").innerHTML += `
      <input type="radio" name="color" id="${color.toLowerCase()}" value="${color
      .toLowerCase()
      .replace(/\s+/g, "")}"/>
      <label for="${color.toLowerCase()}" class="colorPickerRadio"><span class="${color
      .toLowerCase()
      .replace(/\s+/g, "")}"></span></label>
      `;
  }
  ...
  document.getElementById("addToBasket").addEventListener("click", () => {
    if (productSpecs.color != null) {
      ...
      document.getElementById(
        "colorError"
      ).innerHTML = `<span class="m-3 text-success">Ajouté au panier !</span>`;
    } else {
      document.getElementById("colorError").innerHTML = `
